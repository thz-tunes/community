...
async function exibir_seminovos() {
    ...
    try {
        const resposta = await fetch("http://localhost:5000/seminovos");

        const anuncios = await resposta.json();
        var html = "";
        anuncios.forEach(anuncio => {
            const urlImg = anuncio.imagem ? `http://localhost:5000/uploads/${anuncio.imagem}` : 'caminho/para/imagem-padrao.png';
            html += `
...
async function cadastrarAnuncio() {
    try {
        ...
        const requisicao = await fetch("http://localhost:5000/register/seminovo", {
            method: "POST",
            body: form
...
async function cadastrar_usuario(event) {
    ...
    try {
        ...
        const requisicao = await fetch("http://localhost:5000/user/register", {
            method: "POST",
